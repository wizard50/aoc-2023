...
(defn fsm [records groups & [consumed]]
  (loop [i 0
         j 0
         state :START
    ...
    (if (and (< i (count records)) (not result))
      (let [ch (nth records i)
            ...
            result (case next-state
                     :R0 0
                     :R1 1
                     :R2 (+ (fsm (str \# (subs records (inc i))) (nthrest groups j) consumed)
                            (fsm (str \. (subs records (inc i))) (nthrest groups j) consumed))
                     nil)]
...
(defn main []
  (let [text (slurp "day12/sample-input.txt")
    ...
    ; sample
    (println "sample1:" "???.###" [1,1,3] "=>" (fsm "???.###" [1,1,3]))
    (println "sample1:" ".??..??...?##." [1 1 3] "=>" (fsm ".??..??...?##." [1 1 3]))
    (println "sample1:" "?#?#?#?#?#?#?#?" [1 3 1 6] "=>" (fsm "?#?#?#?#?#?#?#?" [1 3 1 6]))
    (println "sample1:" "????.#...#..." [4 1 1] "=>" (fsm "????.#...#..." [4 1 1]))
    (println "sample1:" "????.######..#####." [1 6 5] "=>" (fsm "????.######..#####." [1 6 5]))
    (println "sample1:" "?###????????" [3 2 1] "=>" (fsm "?###????????" [3 2 1]))

    (println "result - sample 1: " (solve text))

    ; solution
    (println "result - part1: " (solve input))))
